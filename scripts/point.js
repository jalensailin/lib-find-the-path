...
export function deg2rad(deg_) {
  return ((deg_ % 360) * Math.PI) / 180;
}
export function rad2deg(r_) {
  return ((180 / Math.PI) * r_) % 360;
}
...
export class Point {
  constructor(data_) {
    if (data_.px) this._x = data_.px / this.scale;
    else if (data_.x) this._x = data_.x;
    else this._x = 0;

    if (data_.py) this._y = data_.py / this.scale;
    else if (data_.y) this._y = data_.y;
    else this._y = 0;
    ...
    const determineNeighborsFunc = () => {
      if (data_.neighborsFunc) return data_.neighborsFunc;
  ...
  static lp(p1_, p2_, p_) {
    if (p_ === MinkowskiParameter.Chebyshev) return Point.Chebyshev(p1_, p2_);
    if (p_ === MinkowskiParameter.Manhattan) return Point.Manhattan(p1_, p2_);
    if (p_ === MinkowskiParameter.Euclidean) return Point.Euclidean(p1_, p2_);
    if (p_ <= 0) return undefined;
    ...
    return Math.pow(
      Math.pow(Math.abs(p1_.x - p2_.x), p_) +
        Math.pow(Math.abs(p1_.y - p2_.y), p_),
      1 / p
    );
  }
  ...
  distToPoint(p_) {
    return Point.lp(this, p_, this.metric);
  }
  ...
  closestNeighborOfToken(target_, rotation_) {
    ...
    for (let n of pf.fromToken(target_).neighbors()) {
      ...
      const rDist = Math.abs(
        this.radialDistToPoint(n, rotation_, AngleTypes.RAD)
      );
  ...
  radialDistToPoint(p_, r_, angleType_) {
    ...
    if (angleType_ === AngleTypes.DEG) return rad2deg(ret);

    return ret;
  }

  radialDistToToken(token_, rotation_, angleType_) {
    return this.radialDistToPoint(
      new PointFactory(this.metric).fromToken(token_),
      rotation_,
      angleType_
    );
  }
  ...
  update(changes_) {
    if (changes_.x !== undefined) this._x = changes_.x / this.scale;
    if (changes_.y !== undefined) this._y = changes_.y / this.scale;
    if (changes_.w !== undefined) this._w = changes_.w / this.scale;
    if (changes_.h !== undefined) this._h = changes_.h / this.scale;
  }
  ...
  // Returns the offset of the point's center, in pixels
  get cpx() {
    return (this.x + 0.5) * this.scale;
  }
  get cpy() {
    return (this.y + 0.5) * this.scale;
  }
  ...
  set metric(minkowskiParameter_) {
    this.metric = minkowskiParameter_;
  }

  // todo: don't do this? return pixel pair instead?
  // Don't use center points for calculations!!
  get center() {
    return PointFactory.fromPoint(this, 0.5, 0.5);
  }
  ...
  get isValid() {
    if (this.x === NaN || this.y === NaN) return false;
    if (this.x < 0 || this.y < 0) return false;
    if (
      this.px >= canvas.dimensions.width ||
      this.py >= canvas.dimensions.height
    )
      return false;
...
export class Segment {
  ...
  distToPoint(point_) {
    return this.distToCoord(point_.x, point_.y);
  }
  ...
  equals(seg_) {
    return this.id === seg_.id;
  }
  // Checks if two segments share an origin
  equalsNoSize(seg_) {
    return this.point.equals(seg_.point);
  }
  ...
  shared(segment_) {
    ...
    for (let point of this.pointSet)
      if (segment_.contains(point)) ret.push(point);
  ...
  get id() {
    return this.point.id + "," + this.width + "," + this.height;
  }
  get isValid() {
    if (!this.point.isValid) return false;
    ...
    if (
      this.point.x + this.width > gridWidth ||
      this.point.y + this.height > gridHeight
    )
      return false;
  ...
  get pw() {
    return this.width * canvas.grid.size;
  }
  get ph() {
    return this.height * canvas.grid.size;
  }
}
...
export class PointFactory {
  ...
  static segmentFromPoint(point_, w_ = 1, h_ = 1) {
    return new Segment(point_, w_, h_);
  }
  ...
  segmentFromCoord(x_, y_, w_ = 1, h_ = 1) {
    return new Segment(this.fromCoord(x_, y_), w_, h_);
  }
  segmentFromData(data_, w_ = 1, h_ = 1) {
    return new Segment(this.fromData(data_), w_, h_);
  }
  segmentFromToken(token_) {
    return new Segment(
      this.fromToken(token_),
