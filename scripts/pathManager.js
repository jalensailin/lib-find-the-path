  ...
  push(item_, priorityMeasure_) {
    ...
    for (let i = 0; i < this.data.length; ++i) {
      ...
      if (curElement.cost === item_.cost) {
        ...
        if (curElement.distToDest < item_.distToDest) continue;
...
export class Path {
  ...
  constructor(data_) {
    const isValidNumber = (thing_) => {
      return thing_ !== undefined && typeof thing_ === "number";
    };
    const isValidPosNumber = (thing_) => {
      return isValidNumber(thing_) && thing_ >= 0;
    };
    ...
    if (data_.origin instanceof Point) {
      if (
        !data_.token &&
        !(isValidPosNumber(data_.width) && isValidPosNumber(data_.height))
      )
        throw "FindThePath | Invalid Path initialization: must provide width/height or token";
    } else if (
      !data_.origin instanceof Segment ||
    ...
    if (data_.origin instanceof Segment) {
      ...
      if (data_.token) this.origin = PointFactory.segmentFromToken(data_.token);
      else
        this.origin = PointFactory.segmentFromPoint(
          data_.origin,
          data_.width,
          data_.height
        );
    ...
    if (data_.dest instanceof Segment) this.dest = data_.dest;
    else this.dest = PointFactory.segmentFromPoint(data_.dest, 1, 1);
    ...
    if (this.token) this._utility.token = this.token;
  ...
  async findPath() {
    ...
    while (frontier.length > 0) {
      ...
      if (
        this._constrainVision &&
        n.prev &&
        !this._utility.losCenter(origin, n.originSeg)
      )
        continue;

      if (n.prev && !this._utility.los(n.prev.originSeg, n.originSeg)) continue;

      if (n.distToDest === 0) {
        if (
          this._utility.collision(
            n.prev.originSeg,
            this._utility.defaultCollisionConfig()
          )
        )
          continue;
      ...
      if (
        n.prev &&
        n.originSeg.pointSet.some((p) => {
          return (
            !n.prev.originSeg.contains(p) &&
            this._utility.collision(n.originSeg)
          );
        })
      ) {
        continue;
      ...
      n.origin
        ...
        .map((p) => {
          return new Node(
            PointFactory.segmentFromPoint(p, width, height),
            n.destSeg,
            n.distTraveled + 1,
            n
          );
        ...
        .filter((node) => {
          if (!node.originSeg.isValid) return false;
  ...
  unwind(node_) {
    ...
    for (let n = node_; n !== null; n = n.prev) this._path.unshift(n);
  }
  ...
  get cost() {
    return this?.terminus.cost;
  }

  get terminalDistanceToDest() {
    return this.terminus?.distToDest;
  }
...
export class PathManager {
  ...
  static async pointsWithinRange(
    seg_,
    dist_,
    collisionConfig_ = { checkCollision: true, whitelist: new Array() }
  ) {
    if (!seg_ instanceof Segment) {
      console.log(
        "FindThePath | Invalid starting segment for call to pointsWithinRange"
      );
      return ret;
    }
    ...
    const getNodeId = (node_) => {
      return node_.origin.id;
    };
    ...
    while (frontier.length > 0) {
      ...
      if (n.prev && !utility.los(n.prev.origin, n.origin)) continue;

      // Tokens with size > 1 have overlap when they move. We don't want them to colide with themselves
      if (
        n.prev &&
        n.origin.pointSet.some((p) => {
          return (
            !n.prev.origin.contains(p) &&
      ...
      if (n.cost > dist_) break;

      const id = getNodeId(n);

      // todo: this is disgusting. optimize...
      if (visited.has(id)) continue;
      else visited.set(id, n);
      ...
      n.origin.point
        ...
        .filter((node) => node.origin.isValid)
        .forEach((node) => {
    ...
    let ret = new Array();
    visited.forEach((v, k, m) => {
      ret.push({ segment: v.origin, dist: v.cost });
    });
    return ret;
  }
  ...
  async addToken(
    ...
    target_,
    movement_,
    config_ = PathManager.defaultConfig()
  ) {
    if (!this._paths.has(token_.id)) this._paths.set(token_.id, new Map());
    ...
    if (tokenPaths.has(target_.id)) {
      console.log(
        "FindThePath | Attempted to add existing target (%s) " +
    ...
    console.log(
      "FindThePath | Searching for path between tokens %s and %s",
      token_.id,
      target_.id
    );
    await p.findPath();
  ...
  clear(tokenId_) {
    this._paths.get(tokenId_).clear();
  }
  ...
  paths(id_) {
    return this._paths.get(id_);
  }
  // Get the path from a token to a target
  path(tokenId_, targetId_) {
    return this.paths(tokenId_)?.get(targetId_);
  }

  async pointsWithinRangeOfToken(token_, dist_) {
    return await PathManager.pointsWithinRange(
      this._pointFactory.segmentFromToken(token_),
      dist_,
...
Hooks.on("ready", () => {
  ...
  game.FindThePath.Chebyshev.PointFactory = new PointFactory(
    MinkowskiParameter.Chebyshev
  );
  game.FindThePath.Chebyshev.PathManager = new PathManager(
    MinkowskiParameter.Chebyshev
  );

  game.FindThePath.Euclidean.PointFactory = new PointFactory(
    MinkowskiParameter.Euclidean
  );
  game.FindThePath.Euclidean.PathManager = new PathManager(
    MinkowskiParameter.Euclidean
  );

  game.FindThePath.Manhattan.PathManager = new PathManager(
    MinkowskiParameter.Manhattan
  );
  game.FindThePath.Manhattan.PointFactory = new PointFactory(
    MinkowskiParameter.Manhattan
  );
