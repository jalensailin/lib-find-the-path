...
export class FTPUtility {
  constructor(data_) {
    ...
    this.collisionConfig = data_?.collisionConfig
      ? data_.collisionConfig
  ...
  collision(newSegment_, collisionConfig_ = this.collisionConfig) {
    ...
    for (let token of canvas.tokens.placeables) {
      ...
      for (let point of tokenSegment.pointSet)
        if (newSegment_.contains(point)) return true;
    }
  ...
  isTraversable(
    oldSegment_,
    newSegment_,
    collisionConfig_ = this.collisionConfig
  ) {
    return (
      !this.collision(newSegment_, collisionConfig_) &&
      this.los(oldSegment_, newSegment_)
    );
  }
  ...
  los(oldSegment_, newSegment_) {
    ...
    if (
      oldSegment_.width !== newSegment_.width ||
      oldSegment_.height !== newSegment_.height
    ) {
      console.log(
        "FindThePath | Invalid LoS comparison between Points of mismatching dimension"
      );
      return false;
    }
    ...
    for (let i = 0; i < oldSegment_.width * oldSegment_.height; ++i) {
      ...
      for (let j = i + 1; j < newSegment_.width * newSegment_.height; ++j)
        if (
          canvas.walls.checkCollision(
            new Ray(p, { x: ps2[j].cpx, y: ps2[j].cpy })
          )
        )
          return false;
    }
  ...
  async moveTokenToSegment(segment_, rotateWait_ = 100) {
    ...
    const dTheta = cur.radialDistToSegment(
      segment_,
      this.tokenDoc.data.rotation,
      AngleTypes.DEG
    );
    ...
    if (dTheta) {
      ...
      await this.tokenDoc
        .update({ rotation: (this.tokenDoc.data.rotation + dTheta) % 360 })
        .then((resolve, reject) => {
          // Wait between rotating and moving
          return new Promise((resolve) =>
            setTimeout(resolve, (dTheta / 360) * rotateWait_)
          );
        });
    }
  ...
  async traverse(distFromEnd_ = 0, rotateWait_ = 100, moveWait_ = 250) {
    ...
    for (let i = 1; i < pathToTraverse.length; ++i) {
      const success = await this.moveTokenToSegment(
        pathToTraverse[i],
        rotateWait_
      );
