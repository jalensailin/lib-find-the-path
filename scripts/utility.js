...
export class FTPUtility {
  constructor(data_) {
    ...
    this.collisionConfig = data_?.collisionConfig ? data_.collisionConfig : this.defaultCollisionConfig();
    this._name = data_?.name ? data_.name : this.defaultName();
    canvas.grid.addHighlightLayer(this._name);
  }

  defaultCollisionConfig() {
    return { checkCollision: true, whitelist: new Array() };
  }
  defaultName() {
    return "FTPUtility";
  }
  ...
  clearHighlights() {
    canvas.grid.highlightLayers[this._name]?.clear();
  }

  collision(newSegment_, collisionConfig_ = this.collisionConfig) {
    if (!collisionConfig_.checkCollision) return false;
    ...
    if (this.tokenDoc) whitelist.push(this.tokenDoc.id);
    for (let id of collisionConfig_.whitelist) whitelist.push(id);

    const checkForToken = (id_, whitelist_) => {
      for (let tokenId of whitelist_) if (id_ === tokenId) return true;

      return false;
    };

    for (let token of canvas.tokens.placeables) {
      if (checkForToken(token.document.id, whitelist)) continue;

      const tokenSegment = pf.segmentFromToken(token);

      for (let point of tokenSegment.pointSet) if (newSegment_.contains(point)) return true;
    }
  ...
  highlightPoints(points_, config_ = { data: {}, retain: false }) {
    if (!config_.retain) this.clearHighlights();
    ...
    points_.forEach((p) => {
      ...
      if (!pointOverlap.has(p)) pointOverlap.set(p, 1);
      else pointOverlap.set(p, pointOverlap.get(p) + 1);
    });

    for (let point of pointOverlap.keys()) canvas.grid.highlightPosition(this._name, { x: point.px, y: point.py });
  }
  highlightSegments(segs_, config_ = { data: {} }) {
    ...
    for (let seg of segs_) {
      if (uniquePoints.has(seg.id)) continue;

      uniquePoints.set(seg.id, 1);
      for (let point of seg.pointSet) points.push(point);
    }
  ...
  los(oldSegment_, newSegment_) {
    if (!oldSegment_ || oldSegment_.equals(newSegment_)) return true;

    if (!newSegment_) return false;
    ...
    if (!oldSegment_.isValid || !newSegment_.isValid) return false;
    ...
    for (let i = 0; i < oldSegment_.width * oldSegment_.height; ++i) {
      if (
        canvas.walls.checkCollision(new Ray({ x: ps1[i].cpx, y: ps1[i].cpy }, { x: ps2[i].cpx, y: ps2[i].cpy }), {
          blockMovement: false,
          blockSenses: true,
          mode: "any",
          type: "sight",
      ...
      for (let j = i + 1; j < newSegment_.width * newSegment_.height; ++j)
        if (
          canvas.walls.checkCollision(new Ray(p, { x: ps2[j].cpx, y: ps2[j].cpy }), {
            blockMovement: false,
            blockSenses: true,
            mode: "any",
            type: "move",
  ...
  losCenter(startSegment_, endSegment_) {
    if (!startSegment_ || startSegment_.equals(endSegment_)) return true;

    if (!endSegment_) return false;

    if (!startSegment_.isValid || !endSegment_.isValid) return false;
  ...
  partialLOS(oldSegment_, newSegment_) {
    ...
    return canvas.walls.checkCollision(
      new Ray({ x: oldSegment_.center.px, y: oldSegment_.center.py }, { x: newSegment_.center.px, y: newSegment_.center.py })
    );
  }
  ...
  async moveTokenToSegment(segment_, rotateWait_ = 100) {
    ...
    if (dTheta) {
      ...
      await this.tokenDoc.update({ rotation: (this.tokenDoc.data.rotation + dTheta) % 360 }).then((resolve, reject) => {
  ...
  async traverse(distFromEnd_ = 0, rotateWait_ = 100, moveWait_ = 250) {
    if (!this.path || !this.path instanceof Path || !this.path.valid) return false;
    // Redundant?
    if (this.path.length === 0) return false;
    if (rotateWait_ < 0 || moveWait_ < 0) return false;
    ...
    if (!start.equals(this.path.path[0].origin)) return false;

    // Fail if the path doesn't get as close as we want
    if (this.path.terminus.distToDest > distFromEnd_) return false;
    ...
    for (let i = 1; i < pathToTraverse.length; ++i) {
      ...
      // Wait between moves
      if (i !== pathToTraverse.length - 1) await new Promise((resolve) => setTimeout(resolve, moveWait_));
    }
  ...
  get tokenDoc() {
    return this._tokenDoc;
  }
  set tokenDoc(tokenDoc_) {
    this._tokenDoc = tokenDoc_;
  }
}
